-- Migration: 20260522000006_seed.sql
-- Deterministic seed: demo institution + superadmin account for prototype demos.
...
begin
  ...
  -- NOTE: auth.identities row is REQUIRED — login fails without it (RESEARCH.md Pitfall 2)
  insert into auth.users (
  ...
  -- auth.identities entry is REQUIRED for email login to work
  ...
  -- Create user_profile (status=approved; active_role=admin for superadmin)
