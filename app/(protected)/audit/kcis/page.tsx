...
// KCI list page — Server Component.
// Shows all KCI definitions with their latest reading status and trend sparkline.
...
import type { IndicatorStatus, IndicatorDirection } from '@/types/kci'
...
type KciRow = {
  ...
  kci_readings: { id: string; actual_value: number; status: IndicatorStatus; period_start: string; period_end: string; recorded_at: string }[] | null
}

export default async function KcisPage() {
  ...
  const { data } = await getKcisWithReadings(supabase)
  const rows = data as unknown as KciRow[]
    ...
    <div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-[10px] border border-dashed border-paper-border bg-paper p-12 text-center">
          <p className="text-[14px] text-navy-mid">No KCIs defined yet.</p>
          {canCreate && (
            <Link href="/audit/kcis/new" className="mt-3 inline-block text-[13px] font-medium text-gold hover:underline">
              Add the first KCI →
            </Link>
          )}
        </div>
        ...
        <div className="overflow-hidden rounded-[10px] border border-paper-border bg-white shadow-card">
          <table className="w-full text-[13px]">
            <thead className="border-b border-paper-border bg-paper text-[12px] font-medium text-navy-mid uppercase tracking-wide">
              <tr>
                <th className="px-4 py-3 text-left">KCI</th>
            ...
            <tbody className="divide-y divide-paper-border">
              {rows.map((row) => {
                const readings = row.kci_readings ?? []
                ...
                return (
                  <tr key={row.id} className="hover:bg-paper/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href={`/audit/kcis/${row.id}`} className="text-[12px] font-medium text-gold hover:underline">
                        View →
                      </Link>
                    </td>
                  </tr>
