'use client'
// app/(protected)/audit/kcis/[id]/RecordReadingForm.tsx — Client Component
import { useState, useTransition } from 'react'
import { useRouter } from 'next/navigation'
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { Loader2 } from 'lucide-react'
import { kciReadingSchema, type KciReadingInput } from '@/lib/schemas/kci'
import { recordKciReading } from '@/lib/audit/kci-actions'
import {
...
export function RecordReadingForm({ kciId }: RecordReadingFormProps) {
  const router = useRouter()
  const [error, setError]            = useState<string | null>(null)
  const [success, setSuccess]        = useState(false)
  const [isPending, startTransition] = useTransition()

  const form = useForm<KciReadingInput>({
    resolver: zodResolver(kciReadingSchema),
    defaultValues: {
      period_start: '',
      period_end:   '',
  ...
  function onSubmit(values: KciReadingInput) {
    setError(null)
    setSuccess(false)
    startTransition(async () => {
      const result = await recordKciReading(kciId, values)
      if ('error' in result) {
